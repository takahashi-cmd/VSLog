...
class BarRenderer:
    def render(self, ax, data: PlotData, *, verticalAxis: VerticalAxis) -> None:
        bottom = [0] * len(data.labels)
        ax.bar(data.labels, data.values[], bottom=bottom, label=data.fieldname, color=data.colors[])
        ax.grid(True)
        ax.set_xlabel('年月日')
        ax.set_ylabel('学習時間（時間）' if verticalAxis == 'time' else '学習時間（%）')
        ymax_val = max(data.values.values()) if data.values else 0
